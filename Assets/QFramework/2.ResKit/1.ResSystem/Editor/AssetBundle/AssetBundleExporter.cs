...
namespace QFramework
{
    public class AssetBundleExporter
    {
		public static void BuildDataTable()
		{
		    ...
		    string filePath =
		        (FilePath.StreamingAssetsPath + ResKitConfig.RELATIVE_AB_ROOT_FOLDER).CreateDirIfNotExists() +
		        ResKitConfig.EXPORT_ASSETBUNDLE_CONFIG_FILENAME;
			table.Save(filePath);
...
#region 指定具体文件构建
        ...
        private static void BuildAssetBundlesInFolder(string folderPath)
        {
			...
			BuildPipeline.BuildAssetBundles(ResKitConfig.EDITOR_AB_EXPORT_ROOT_FOLDER,
